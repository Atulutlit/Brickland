...
const { propertyCategoryModel } = require("../model/categoryModel");
const {
...
const addCategory = async function (req, res) {
  try {
    let { type, categoryName, slug, categoryImg, description, url, parentId } =
      req.body;
    const checkCategoryData = await propertyCategoryModel.findOne({
      categoryName,
    ...
    if (slug) {
      const checkSlugData = await propertyCategoryModel.findOne({ slug });
      if (checkSlugData) {
        slug = slug + "-1";
      }
    }
    let checkCategoryId;
    if (parentId) {
      checkCategoryId = await propertyCategoryModel.findOne({
    ...
    const addCategory = await propertyCategoryModel.create(addObj);
    if (addCategory) {
      return res.json({
...
const categoryList = async function (req, res) {
  try {
    ...
    const categoryListData = await propertyCategoryModel
      .find(findQuery)
      .sort({ createdAt: -1 })
      .limit(contentPerPage)
      .skip(contentPerPage * page - contentPerPage);
    if (categoryListData.length) {
      const total = await propertyCategoryModel.countDocuments(findQuery);
      return res.json({
...
const subCategoryList = async function (req, res) {
  try {
    ...
    const subCategoryListData = await propertyCategoryModel.aggregate([
      {
        $match: {
          type: "PARENT",
          ...(categoryId && {
...
const getCategoryById = async function (req, res) {
  try {
    ...
    const categoryData = await propertyCategoryModel.findOne({
...
const updateCategory = async function (req, res) {
  try {
    ...
    const findCategory = await propertyCategoryModel.findOne(findQuery);
    if (!findCategory) {
      return res.json({
        meta: { msg: "Category not found", status: false },
      });
    ...
    if (categoryName) {
      const findCategory = await propertyCategoryModel.findOne({
        categoryName,
        _id: new Types.ObjectId(_id),
      ...
      if (!findCategory) {
        const findCategoryAgain = await propertyCategoryModel.findOne({
          categoryName,
          ...(parentId && { type: checkCategoryId.type }),
          ...(!parentId && { type: "ROOT" }),
        });
    ...
    if (slug) {
      const checkSlugData = await propertyCategoryModel.findOne({
        slug,
        _id: new Types.ObjectId(_id),
      });
      if (!checkSlugData) {
        const checkSlugDataAgain = await propertyCategoryModel.findOne({ slug });
    ...
    const updatedCategoryData = await propertyCategoryModel.findOneAndUpdate(
      findQuery,
      { $set: updateQuery },
      { new: true }
    );
...
const changeCategoryStatus = async (req, res) => {
  try {
    ...
    let updateData = await propertyCategoryModel.findOneAndUpdate(
      { _id: new Types.ObjectId(_id) },
...
const deleteCategory = async (req, res) => {
  try {
    ...
    if (findCategory.type === "ROOT") {
      const findSubCategory = await propertyCategoryModel.find({
    ...
    const deleteCategory = await propertyCategoryModel.deleteOne({
      _id: new Types.ObjectId(_id),
    });
