...
const { blogCategoryModel } = require("../model/categoryModel");
const {
  getCustomBlogCategoryId,
...
const addCategory = async function (req, res) {
  try {
    let { type, categoryName, slug, categoryImg, description, url, parentId } =
      req.body;
    const checkCategoryData = await blogCategoryModel.findOne({
      categoryName,
    ...
    if (slug) {
      const checkSlugData = await blogCategoryModel.findOne({ slug });
      if (checkSlugData) {
        slug = slug + "-1";
      }
    }
    let checkCategoryId;
    if (parentId) {
      checkCategoryId = await blogCategoryModel.findOne({
    ...
    if (type && type.toUpperCase() === "ROOT") {
      categoryId = await getCustomBlogCategoryId();
    } else {
    ...
    const addCategory = await blogCategoryModel.create(addObj);
    if (addCategory) {
      return res.json({
...
const categoryList = async function (req, res) {
  try {
    ...
    const categoryListData = await blogCategoryModel
      .find(findQuery)
      .sort({ createdAt: -1 })
      .limit(contentPerPage)
      .skip(contentPerPage * page - contentPerPage);
    if (categoryListData.length) {
      const total = await blogCategoryModel.countDocuments(findQuery);
      return res.json({
...
const subCategoryList = async function (req, res) {
  try {
    ...
    const subCategoryListData = await blogCategoryModel.aggregate([
      {
        $match: {
          type: "PARENT",
          ...(categoryId && {
...
const getCategoryById = async function (req, res) {
  try {
    ...
    const categoryData = await blogCategoryModel.findOne({
...
const updateCategory = async function (req, res) {
  try {
    ...
    const findCategory = await blogCategoryModel.findOne(findQuery);
    if (!findCategory) {
      return res.json({
        meta: { msg: "Category not found", status: false },
      });
    ...
    if (categoryName) {
      const findCategory = await blogCategoryModel.findOne({
        categoryName,
        _id: new Types.ObjectId(_id),
      ...
      if (!findCategory) {
        const findCategoryAgain = await blogCategoryModel.findOne({
          categoryName,
          ...(parentId && { type: checkCategoryId.type }),
          ...(!parentId && { type: "ROOT" }),
        });
    ...
    if (slug) {
      const checkSlugData = await blogCategoryModel.findOne({
        slug,
        _id: new Types.ObjectId(_id),
      });
      if (!checkSlugData) {
        const checkSlugDataAgain = await blogCategoryModel.findOne({ slug });
    ...
    const updatedCategoryData = await blogCategoryModel.findOneAndUpdate(
      findQuery,
      { $set: updateQuery },
      { new: true }
    );
...
const changeCategoryStatus = async (req, res) => {
  try {
    ...
    let updateData = await blogCategoryModel.findOneAndUpdate(
      { _id: new Types.ObjectId(_id) },
...
const deleteCategory = async (req, res) => {
  try {
    ...
    if (findCategory.type === "ROOT") {
      const findSubCategory = await blogCategoryModel.find({
    ...
    const deleteCategory = await blogCategoryModel.deleteOne({
      _id: new Types.ObjectId(_id),
    });
